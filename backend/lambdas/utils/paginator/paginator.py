...
from src.lambdas.utils.paginator.exceptions import PageNotAnInteger, EmptyPage
...
class Paginator(object):
    ...
    records from database against given queryset. Optimization will be applied
    only on the query that will be used for fetching total number of records.
    You can also provide the separate query in optional_count_queryset argument
    ...
    ..usage::
        ...
        :Example1:
        >>> from src.lambdas.utils.paginator.paginator import Paginator
	...
	def __init__(self, queryset, per_page_limit, optional_count_queryset=None,
	             allow_empty_first_page=True):
		"""
        Constructor to create the paginator object.

        :param queryset: SQLAlchemy query which is used for fetching data from
                          from database.
        :type queryset: SQLAlchemy query object.
        :param per_page_limit: Required number of records in a page.
        :type per_page_limit: int.
        :param optional_count_queryset: This is a optional query set that will
                                         use to fetch the total number of
                                         records from database. If this optional
                                         query is not provided than this class
                                         will optimized queryset query and used
                                         that optimized query of getting total
                                         number of records from database.
        :type optional_count_queryset: SQLAlchemy query object.
		...
		self.optional_count_queryset = optional_count_queryset
	...
	def page(self, page_number):
		...
		return Page(self.queryset.offset(offset).limit(self.per_page_limit).all(),
		            page_number, self)

	def __get_count(self):
		self.__count = self.queryset.count()
		return self.__count
